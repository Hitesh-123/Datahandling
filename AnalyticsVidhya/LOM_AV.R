...
Model1 = randomForest(is_open ~ communication_type + total_links + no_of_internal_links + no_of_images + no_of_sections + email_body + subject + email_url, data = comboset, nodesize = 5)
summary(Model1)
plot(Model1)
IS_open = predict(Model1, newdata = comboset2, type = "class")
summary(IS_open)
comboset2$is_open = IS_open
str(comboset2)
Model2 = randomForest(Click ~ is_open + communication_type + total_links + no_of_internal_links + no_of_images + no_of_sections + email_body + subject + email_url, data = comboset, nodesize = 5)
summary(Model2)
plot(Model2)
pred1 = predict(Model2, newdata = comboset2, type = "class")
summary(pred1)
...
write.csv(Solution,file = "LOM.csv", row.names = F)
